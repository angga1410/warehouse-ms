...
@include('flash::message')
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h3 class="m-subheader__title m-subheader__title--separator">Dispatch to Warehouse</h3>
            </div>
        </div>
    </div>
    <div class="m-content">
...
<!-- <div class="col-xl-4 col-lg-5"> -->
  <div class="card shadow mb-4">
    ...
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      <h4 class="m-0 font-weight-bold text-danger">Outstanding QC</h4>
        ...
        $("#qc_request").DataTable( {
            ...
            },
            {
                data: function(data){
                    if(data.status == 0)
                    {
                       return '<span class="m-badge m-badge--warning m-badge--wide m--font-light">On Progress</span>';
                    }else if(data.status == 1)
                    {
                       return '<span class="m-badge m-badge--info m-badge--wide m--font-light">Cancelled</span>';
            ...
            // },
            {
                ...
                data: function(data){
                    return '<a style="text-decoration: none;" href="#" data-toggle="modal" data-target="#m_modal_2" class="view btn btn-outline-accent m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--outline-2x m-btn--pill m-btn--air btn-sm"><i class="la la-eye"></i></a> <a href="/edit-qcRequest/'+data.id+'" class="btn btn-outline-brand m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--outline-2x m-btn--pill m-btn--air btn-sm"><i class="la la-pencil"></i></a> <a href="#" class="delete btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--pill m-btn--air btn-sm"><i class="la la-close"></i></a>';
                }
            },
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        $.each( data, function( key, value ) {
                          ...
                          $(".qcitem_parts_tbl").append('<tr class="detail_tr"><td>'+value.products.mfr+'</td><td>'+value.products.part_num+'</td> <td>'+value.products.part_name+'</td><td>'+value.products.part_desc+'</td> <td>'+value.qty_qc+'</td> <td>'+value.products.default_um+'</td></tr>');
            ...
            // $("#qc_request .edit").on("click",function(e){
            //     e.preventDefault();
            //     var input = $('#qc_request').DataTable().row($(this).parents('tr')).data();
            //     $.ajax({
            //         url:"/edit-qcRequest",
